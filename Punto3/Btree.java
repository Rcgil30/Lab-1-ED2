...
public class Btree {
    ...
    public Btree(int t) {
        ...
        child = new Btree[(2 * t)];  //cada nodo soporta 2t hijos, por definción 
        leaf = true; //hoja
        Nclaves = 0;  
    }
    ...
    public void PrintClaves() { //esto es una manera simple de guiarnos al momento de imprimir los nodos
        for (int i = 0; i < Nclaves; i++) {  //se imprime cada clave del nodo
            if (i < Nclaves - 1) {  //imprimos las claves, hasta la penultima Nclaves -1 para que no quede la coma
